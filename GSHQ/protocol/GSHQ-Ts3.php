...
function GSHQ_ts3_engine(array $server, array $viewer_data): array {
    ...
    // 🔐 Connexion avec login ServerAdmin
    $login = $send("login ServerAdmin pw123");
    if (!str_contains($login, 'error id=0')) {
        $viewer_data['hostname'] = "❌ Login Query TS3 (ServerAdmin) échoué";
        fclose($sock);
        return $viewer_data;
    }
    ...
    $info = $ts3_parse($resp_info);

    // Parse clients
    $clients = [];
    foreach (explode('|', trim($resp_clients)) as $entry) {
        $data = $ts3_parse($entry);
    ...
    // Traitement final
    $hostname       = htmlspecialchars($info['virtualserver_name'] ?? '❓ Serveur inconnu', ENT_QUOTES, 'UTF-8');
    ...
    $viewer_data['protocol']      = 'ts3';
    $viewer_data['debug_raw']     = $resp_info;
