...
$games = $server['games'];
$adress_ip = $server['adress_ip'];
$c_port = $server['c_port'];
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['form1'])) {
	...
	if ($games !== 'discord' && empty($q_port)) {
		$valid = false;
		$error_message .= 'Query Port is required<br>';
	}

	if ($valid) {
		try {
			$stmt = $conn->prepare("UPDATE tbl_gameserver SET games = ?, adress_ip = ?, c_port = ?, q_port = ?, rcon_port = ?, rcon_password = ? WHERE id = ?");
			$stmt->execute([$games, $adress_ip, $c_port, $q_port, $rcon_port, $rcon_password, $id]);

			$success_message = 'Server successfully updated.';
		} catch (PDOException $e) {
			$error_message = "Error while updating the server: " . $e->getMessage();
		}
	}
}
?>
			...
			<div class="card-body">
				...
				<form action="" method="post">
									...
									<div class="row">
										...
										<div class="col-md-4">
											<div class="form-group">
												...
												<input type="text" class="form-control" name="adress_ip" value="<?= htmlspecialchars($adress_ip) ?>">
											</div>
										</div>
										<div class="col-md-4" id="c_port_container">
											<div class="form-group">
												<label for=""><b class="text-muted d-block">Connection Port *</b></label>
												<input type="text" class="form-control" name="c_port" value="<?= htmlspecialchars($c_port) ?>">
											</div>
										</div>
										<div class="col-md-4" id="q_port_container">
											<div class="form-group">
												<label for=""><b class="text-muted d-block">Default Query Port *</b></label>
												<input type="text" class="form-control" name="q_port" value="<?= htmlspecialchars($q_port) ?>">
											</div>
										</div>										
									</div>
								</div>
									...
									<div class="row">
										...
										<div class="col-md-6">	
											<div class="form-group">
												...
												<input type="text" class="form-control" name="rcon_port" value="<?= htmlspecialchars($rcon_port) ?>">
											</div>										
										</div>
										<div class="col-md-6">	
											<div class="form-group">
												<label for=""><b class="text-muted d-block">Rcon password</b></label>
												<input type="password" class="form-control" name="rcon_password" value="<?= htmlspecialchars($rcon_password) ?>">
											</div>											
										</div>										
									</div>
								</div>
					...
					<div class="d-block mt-2">
						<center><button type="submit" class="btn btn-primary mt-2 pr-4 pl-4" name="form1">Edit my gameserver</button></center>
					</div>					
				</form>
			</div>
		</div>
